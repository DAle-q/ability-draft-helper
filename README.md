...
Все 60 координат можно править без изменения кода в `data/layout.json`. Номер позиции — порядок записи в массиве `slots`: 1–12 — ультимейты, 13–60 — обычная сетка. В каждой записи отдельно меняются `x` и `y`; строки идут сверху вниз, позиции в строке — слева направо. После сохранения файла перезапусти помощник или сделай новый снимок.
